...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store the returned JWT token in localStorage as requested
      if (data.accessToken) {
        localStorage.setItem('token', data.accessToken);
        
        // Also update the auth store if available
        if (data.user) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-4 sm:px-6 lg:px-8 font-sans">
      <div className="w-full max-w-[400px] bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        ...
        <form onSubmit={handleLogin}>
          ...
          <div className="mb-5">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link href="/forgot-password" className="text-sm font-medium text-[#534AB7] hover:underline transition-all">
                Forgot password?
              </Link>
            </div>
            <div className="relative">
        ...
        <div className="mt-8 text-center text-sm text-gray-500">
          ...
          <Link href="/register" className="font-medium text-[#534AB7] hover:underline transition-all">
            Sign up
          </Link>
        </div>
